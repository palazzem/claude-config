...
if printf '%s' "$cmd" | grep -qiE 'generated with claude|co-authored-by:.*claude|noreply@anthropic\.com'; then
  echo "commit-guard: blocked - commit text contains Claude/Anthropic attribution (Generated with Claude, Co-Authored-By: ...claude, or an Anthropic noreply email); remove it and commit again." >&2
  exit 2
fi

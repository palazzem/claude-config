...
4. **Watch to merge** — A monitor owned by the main session fires on comments, human reviews, CI, and pushes; its only action is waking the builder, which responds to feedback, fixes CI, and rebases. The main session reclaims the branch and worktree once the PR reaches its end state. Final review and merge are always yours.
...
| Agent | Role |
|---|---|
| `builder` | Persistent per PR: implements from the spec, opens the draft PR, fixes or rebuts review findings, keeps every push green through CI, then services the open PR (feedback, rebases). The main session, not the builder, reclaims its worktree and branch. |

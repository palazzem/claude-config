...
1. **Understand** (`brainstorming`) — Two gated stages, Understanding then Design Overview, turn a raw request into an approved spec. Each stage is red-teamed by the skeptic and needs your explicit approval. The spec is written in the conversation, and you choose where it goes from there: a GitHub issue, a markdown document via its own PR, or straight into implementation.
...
3. **Review** (`review-panel`) — A blind panel of persona reviewers, selected by the impact of the change, is spawned once per PR; the skeptic verifies findings; survivors post as one batched inline review, each finding marked fix-now or deferred so a PR stays scoped to one concern. The builder fixes or rebuts, and the same reviewers are woken to re-check — three rounds without convergence stops the loop and escalates with a diagnosis. Then the PR flips open and you get one of two messages: "ready for your final review - push back or merge" or "escalated items - findings pushed back with uncertainty, your call". The panel also runs standalone on any PR.
4. **Watch to merge** — A monitor owned by the main session fires on comments, human reviews, CI, and pushes; its only action is waking the builder, which responds to feedback, fixes CI, and rebases when the base moves. Every push stays the builder's until its checks are green. On merge it files one tracking issue per deferred finding and sends a final report; it tears nothing down, since the harness owns the worktree and GitHub deletes the remote branch. Final review and merge are always yours.
...
Two capabilities are kept but deactivated: `visual-evidence` (frontend visual proof) and the Graphite stacking prompt.

## Agents

| Agent | Role |
|---|---|
| `builder` | Persistent per PR: implements from the spec, opens the draft PR, fixes or rebuts review findings, owns every push until its checks are green, then services the open PR (feedback, CI, rebases) and closes it out on merge by filing each deferred finding as a tracking issue. |
...
- PR stacking is deliberately not enabled yet; the ready-to-apply prompt that reintroduces it is kept with the other deactivated skills, for once Graphite (`gt`) is adopted

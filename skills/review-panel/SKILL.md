...
| Persona | Lens |
|---|---|
| `fresh-eyes` | first-look senior engineer: bugs, edge cases, error handling, clarity |
| `breaker` | failure hypotheses: malformed input, dependencies down, scale, rollout, observability |
| `tests` | do the tests protect the code; TDD evidence for bugfixes |
| `standards` | house rules: comments, docstrings, suppressions, DRY, naming |
| `correct-design` | is this the right design; conformance to the approved spec |
| `security-audit` | deep security: data-flow trace and concrete exploit per finding |
| `database` | data stores, migrations, query cost, schema coordination |
| `api-contracts` | contract shapes, serialization, deploy-window coexistence |
| `concurrency` | races, ordering, idempotency of side effects |
| `frontend` | UI states, client/server validation split, reactivity, accessibility |
| `infra` | deployment config, IaC, rollout safety, monitoring |
| `reliability` | retries, timeouts, resource bounds, cache invalidation |
| `security-breadth` | broad OWASP-style sweep; second security lens |

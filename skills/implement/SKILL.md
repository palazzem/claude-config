...
**Hard rule — one builder per PR, forever.** Every later fix goes to the same agent via SendMessage. Never spawn a second builder or a fresh fixer for a PR that has one; the builder's accumulated context is the point.

## 3. The review loop (max 3 rounds)

With `round` starting at 1:

1. **Panel.** Invoke the review-panel skill by name on the draft PR (round 1 spawns the panel; later rounds wake it). This skill contains no panel logic — selection, verification, and posting are entirely review-panel's.
2. **Fixes.** If the round posted findings, wake the builder: "address the panel review on PR #N". The builder verifies each finding, fixes or rebuts on the threads, pushes, and reports back.
3. **Re-round.** Only if the builder applied substantive fixes — logic or design changes, not mechanical ones (lint, renames, comment edits) — increment `round` and go to 1, asking review-panel for a re-check. Hard cap: 3 rounds total, then terminal regardless of state.

## 4. Open flip and the one message

When the loop converges or the cap is hit:

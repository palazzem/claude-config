...
4. Builds and test runs finish clean. Every compiler, linter, test-runner, deprecation, or build warning your change introduces or surfaces is fixed at its cause - never suppressed, filtered, or baselined. A warning you cannot fix, including one from a dependency, is justified by name in the PR body rather than left unmentioned.

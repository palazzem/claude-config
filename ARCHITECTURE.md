...
| `shepherd` | Owns the human-facing surface of a PR: analyzes the diff and writes the title, body, and labels at creation, and refreshes the description when the builder judges it has drifted. Spawned fresh per invocation with no state between them; never commits, pushes, or changes PR lifecycle. |
